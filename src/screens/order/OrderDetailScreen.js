                    ...
                    <ScrollView contentContainerStyle={invM.scroll} showsVerticalScrollIndicator={false}>
                        ...
                        <View style={[invM.card, { backgroundColor: isDark ? '#1C1A14' : '#FFFFFF', borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.07)' }]}>
                            <View style={invM.invIdRow}>
                                ...
                                <Text style={[invM.invNumber, { color: C.primary }]}>#{invoice.invoiceNumber}</Text>
                            </View>
                            <Text style={[invM.invDate, { color: C.textMuted }]}>Invoice Date: {fmtDate(invoice.invoiceDate)}</Text>
                            {invoice.paymentDetails?.paymentDate && (
                                <Text style={[invM.invDate, { color: C.textMuted }]}>Payment Date: {fmtDate(invoice.paymentDetails.paymentDate)}</Text>
                            )}
                            {invoice.paymentDetails?.razorpayPaymentId && (
                                <Text style={[invM.txnId, { color: C.textMuted }]}>Txn: {invoice.paymentDetails.razorpayPaymentId}</Text>
                            )}
                        </View>
                        ...
                        <View style={[invM.card, { backgroundColor: isDark ? '#1C1A14' : '#FFFFFF', borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.07)' }]}>
                            ...
                            <Text style={[invM.customerName, { color: C.textPrimary }]}>{invoice.customerDetails?.name}</Text>
                            {invoice.customerDetails?.email && <Text style={[invM.customerDetail, { color: C.textSecondary }]}>{invoice.customerDetails.email}</Text>}
                            {invoice.customerDetails?.contactNo && <Text style={[invM.customerDetail, { color: C.textSecondary }]}>{invoice.customerDetails.contactNo}</Text>}
                            {invoice.customerDetails?.city && <Text style={[invM.customerDetail, { color: C.textSecondary }]}>{invoice.customerDetails.city}</Text>}
                        </View>
                            ...
                            <View style={{ flexDirection: 'row', gap: 8, marginTop: 4 }}>
                                {invoice.vehicleDetails?.cc && (
                                    <View style={[invM.tag, { backgroundColor: isDark ? '#2A2318' : '#F0EDE6' }]}>
                                        <Text style={[invM.tagText, { color: C.textSecondary }]}>{invoice.vehicleDetails.cc}cc</Text>
                                    </View>
                                )}
                                {invoice.vehicleDetails?.bs && (
                                    <View style={[invM.tag, { backgroundColor: isDark ? '#2A2318' : '#F0EDE6' }]}>
                                        <Text style={[invM.tagText, { color: C.textSecondary }]}>{invoice.vehicleDetails.bs}</Text>
                                    </View>
                                )}
                            </View>
                        </View>
                        ...
                        {invoice.serviceProvided?.length > 0 && (
                            <View style={[invM.card, { backgroundColor: isDark ? '#1C1A14' : '#FFFFFF', borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.07)' }]}>
                                <Text style={[invM.sectionLabel, { color: C.textMuted }]}>SERVICES</Text>
                                {invoice.serviceProvided.map((s, i) => (
                                    <View key={i} style={invM.lineItem}>
                                        <View style={{ flex: 1 }}>
                                            <Text style={[invM.itemName, { color: C.textPrimary }]}>{s.serviceName}</Text>
                                            {s.quantity > 1 && <Text style={[invM.itemQty, { color: C.textMuted }]}>Qty: {s.quantity}</Text>}
                                        </View>
                                        <Text style={[invM.itemPrice, { color: C.textPrimary }]}>{fmtCurrency(s.discountPrice || s.price)}</Text>
                                    </View>
                                ))}
                            </View>
                        )}
                        ...
                        {invoice.partsUsed?.length > 0 && (
                            <View style={[invM.card, { backgroundColor: isDark ? '#1C1A14' : '#FFFFFF', borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.07)' }]}>
                                <Text style={[invM.sectionLabel, { color: C.textMuted }]}>PARTS USED</Text>
                                {invoice.partsUsed.map((p, i) => (
                                    <View key={i} style={invM.lineItem}>
                                        <View style={{ flex: 1 }}>
                                            <Text style={[invM.itemName, { color: C.textPrimary }]}>{p.partName}</Text>
                                            {p.quantity > 1 && <Text style={[invM.itemQty, { color: C.textMuted }]}>Qty: {p.quantity}</Text>}
                                        </View>
                                        <Text style={[invM.itemPrice, { color: C.textPrimary }]}>{fmtCurrency(p.discountPrice || p.price)}</Text>
                                    </View>
                                ))}
                            </View>
                        )}
                        ...
                        {/* Payment Summary */}
                        <View style={[invM.card, { backgroundColor: isDark ? '#1C1A14' : '#FFFFFF', borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.07)' }]}>
                            <Text style={[invM.sectionLabel, { color: C.textMuted }]}>PAYMENT SUMMARY</Text>
                            {invoice.total?.baseAmount > 0 && (
                                <View style={invM.summaryRow}>
                                    <Text style={[invM.summaryLabel, { color: C.textSecondary }]}>Subtotal</Text>
                                    <Text style={[invM.summaryValue, { color: C.textSecondary }]}>{fmtCurrency(invoice.total.baseAmount)}</Text>
                                </View>
                            )}
                            {invoice.total?.discount > 0 && (
                                <View style={invM.summaryRow}>
                                    <Text style={[invM.summaryLabel, { color: '#2ECC9A' }]}>Discount</Text>
                                    <Text style={[invM.summaryValue, { color: '#2ECC9A' }]}>-{fmtCurrency(invoice.total.discount)}</Text>
                                </View>
                            )}
                            {invoice.total?.referralDiscount > 0 && (
                                <View style={invM.summaryRow}>
                                    <Text style={[invM.summaryLabel, { color: '#2ECC9A' }]}>Referral Discount</Text>
                                    <Text style={[invM.summaryValue, { color: '#2ECC9A' }]}>-{fmtCurrency(invoice.total.referralDiscount)}</Text>
                                </View>
                            )}
                            {invoice.total?.sgst > 0 && (
                                <View style={invM.summaryRow}>
                                    <Text style={[invM.summaryLabel, { color: C.textMuted }]}>SGST ({invoice.total.sgstRate}%)</Text>
                                    <Text style={[invM.summaryValue, { color: C.textMuted }]}>{fmtCurrency(invoice.total.sgst)}</Text>
                                </View>
                            )}
                            {invoice.total?.cgst > 0 && (
                                <View style={invM.summaryRow}>
                                    <Text style={[invM.summaryLabel, { color: C.textMuted }]}>CGST ({invoice.total.cgstRate}%)</Text>
                                    <Text style={[invM.summaryValue, { color: C.textMuted }]}>{fmtCurrency(invoice.total.cgst)}</Text>
                                </View>
                            )}
                            <View style={[invM.divider, { backgroundColor: C.border }]} />
                            <View style={invM.summaryRow}>
                                <Text style={[invM.totalLabel, { color: C.textPrimary }]}>TOTAL PAID</Text>
                                <Text style={[invM.totalValue, { color: '#2ECC9A' }]}>
                                    {fmtCurrency(invoice.paymentDetails?.amountPaid || invoice.total?.finalPayable)}
                                </Text>
                            </View>
                        </View>
...
const invM = StyleSheet.create({
    ...
    invDate: { fontSize: 12, letterSpacing: 0.1 },
    txnId: { fontSize: 10, letterSpacing: 0.3 },
    ...
    lineItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', paddingVertical: 6 },

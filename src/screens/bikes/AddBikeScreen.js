// screens/bikes/AddBikeScreen.js
// Wires AddBikeForm → useBikes.createBike → Alert → goBack
    ...
    return (
        ...
        >
            <View style={[stripStyles.iconWrap, { backgroundColor: theme.colors.primary }]}>
                <MaterialCommunityIcons name="motorbike" size={26} color="#1a1a1a" />
            </View>
            <View style={stripStyles.textBlock}>
                ...
                <Text style={[stripStyles.sub, { color: theme.colors.textMuted }]}>
                    Fill in your bike details below
                </Text>
            </View>
        </Animated.View>
    );
    ...
    const handleSubmit = async (bikeData) => {
        ...
        try {
            ...
            setPopup({
                ...
                type: 'success',
                title: 'Bike Added 🏍️',
                message: `${bikeData.brand} ${bikeData.model} has been added to your garage!`,
            });
        } catch (err) {
            setPopup({
                visible: true,
                type: 'error',
                title: 'Error',
                message: err.message || 'Failed to add bike. Please try again.',
            });
        } finally {
            setSubmitting(false);
        }
